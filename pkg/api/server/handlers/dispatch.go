...
func Dispatch(env *env.Env, rtr *mux.Router) *mux.Router {
	...
	rtr.Handle("/appUser", mwr.Adapt(Handler{env, CreateUserHandler}, mwr.LogRequest(env, audit), mwr.LogResponse(env, audit))).
		Methods("POST").
		Headers("Content-Type", "application/json")

	// match only POST requests on /api/v1/appUser/create
	rtr.Handle("/v1/appUser", mwr.Adapt(Handler{env, CreateUserHandler}, mwr.LogRequest(env, audit), mwr.LogResponse(env, audit))).
		Methods("POST").
		Headers("Content-Type", "application/json")

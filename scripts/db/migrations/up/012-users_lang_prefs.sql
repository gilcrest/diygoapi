...
(
    ...
    constraint users_lang_prefs_pk
        primary key (user_id, language_tag),
    constraint users_lang_prefs_user_id_fk
        foreign key (user_id) references users,
    constraint users_lang_prefs_create_app_fk
        foreign key (create_app_id) references app
            deferrable initially deferred,
    constraint users_lang_prefs_update_app_fk
        foreign key (update_app_id) references app
            deferrable initially deferred,
    constraint users_lang_prefs_create_user_fk
        foreign key (create_user_id) references users
            deferrable initially deferred,
    constraint users_lang_prefs_update_user_fk

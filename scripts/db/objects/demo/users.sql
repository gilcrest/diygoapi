...
(
    ...
    constraint users_pk
        primary key (user_id),
    constraint users_extl_id_ui
        unique (user_extl_id),
    constraint users_create_app_fk
        foreign key (create_app_id) references app
            deferrable initially deferred,
    constraint users_create_self_ref_fk
        foreign key (create_user_id) references users
            deferrable initially deferred,
    constraint users_update_app_fk
        foreign key (update_app_id) references app
            deferrable initially deferred,
    constraint users_update_self_ref_fk

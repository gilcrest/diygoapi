...
func (e Oauth2TokenExchange) Exchange(ctx context.Context, realm string, provider diy.Provider, token *oauth2.Token) (*diy.ProviderInfo, error) {
	switch provider {
	case diy.Google:
...
// populates a diy.ProviderInfo based on the response.
func googleTokenExchange(ctx context.Context, realm string, token *oauth2.Token) (*diy.ProviderInfo, error) {
	var (
